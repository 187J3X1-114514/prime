...
它提供可切换的性能/质量实时 Wavefront 积分器、时空重建，以及独立的原生分辨率累积截图
模式。
...
- 两套实时 1 spp Wavefront 积分器：性能模式采用有界单路径和仅太阳 NEE，质量模式保留
  长尾路径、灯光树和完整局部灯采样；
...
- **Prime 路径追踪**：在原版和 Prime 之间切换。关闭后不会继续运行或保留 Prime 的
  路径追踪、降噪和超分资源。
- **实时积分器**：性能模式是默认值，采用可调的有界单路径、仅主动采样太阳；质量模式
  保留长尾路径、灯光树及太阳和局部发光面的完整 NEE。两者使用相同的首透明面棋盘格
  单分支和独立重建探针。
...
- **性能模式最大反弹次数**：限制性能积分器的表面顶点数，可选 1–12，默认 6；主光线
  首次命中计为第 1 次。
...
实时积分器和重建质量的默认值都设为“性能”，只是为了让较低端但满足功能要求的设备在
首次启动或恢复默认值后不容易因资源与调度开销而严重卡顿，并不代表对所有设备推荐这个
具体档位。请根据 GPU、输出分辨率、目标帧率、场景和材质包自行调整积分器、反弹上限与
重建质量。
...
- 静态二次幂 cutout 纹理在 256×256 以内使用匹配分辨率的二值 OMM；更高分辨率按已知上限
  近似，极细透明边界可能错误；
- 两种实时模式都在相机首透明面以 50/50 时空棋盘格选择一条条件反射或条件透射路径，后续
  ...
  只沿原连接线累计体积吸收；
- 性能模式不主动采样方块灯和其他局部发光面，它们只在路径实际命中时贡献，因此局部灯光
  为主的场景通常比质量模式收敛更慢；
- 实体玻璃与水都占用介质栈。透明边界需有
