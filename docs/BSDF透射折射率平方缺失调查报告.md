...
> 历史说明：本文记录旧版物理透射闭包及固定 RoboCute 参考闭包的调查。当前 Prime
> 主路径在适配层恢复了确定性 Snell 方向和 `relativeEta`，但没有恢复参考闭包的透射
> throughput、随机粗糙折射或玻璃介质栈。玻璃仍是逐面滤色器，水仍按路径距离做体积吸收；
> 因此下述 `/η²` 结论对参考闭包和历史实现仍有回归价值，但不应解释为当前表面滤色
> throughput 的生产语义。
