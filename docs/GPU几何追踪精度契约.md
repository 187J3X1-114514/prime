...
`rayOrigin + rayDirection * RayTCurrent()` 代替。几何法线由同一三角形的两条边计算，并按
authored outward normal 定向。只有叉积长度平方精确等于零的退化三角形才能回退 authored
normal，不得用非零阈值吞掉面积很小但有效的几何。
...
大气坐标、太阳壳层和 SHARC 等独立数值域有各自的尺度与边界规则；其中出现的常量不能移植为
几何追踪 epsilon。
