...
- 每像素每帧只追踪一条完整 BSDF 路径，并只运行一套 NRD 4.17.4 `REBLUR_DIFFUSE_SPECULAR`。PT 反弹循环以 `hitted_non_delta=false` 开始，沿透明 delta 链继续推进；每个 delta 事件只把当前表面的镜面 albedo 乘入独立 guide 累积量，首次采到 non-delta 事件时再乘入该表面的漫反射与镜面 albedo 之和，并锁定降噪法线与线性粗糙度。漫反射与镜面光传输去调制后进入同一套历史，命中距离重建使用 5×5 区域。透明材质不再确定性拆成反射、透射两条路径，也不再拥有额外的降噪实例、虚拟表面历史或透明合成 pass。
