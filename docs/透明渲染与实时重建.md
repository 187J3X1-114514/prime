...
3. **更高级的采样算法超出当前实时预算。** 恢复物理折射链的可用直接光采样，通常需要
   ...
   视距场景预算难以同时承担这些成本。质量模式仍保留现有灯光树、长尾队列和完整 NEE，
   性能模式则保留有界调度与仅太阳 NEE。
...
- 槽 0：按几何法线与界面 IOR 确定方向、响应为 `(1-F) / η²` 的折射路径；
- 槽 1：条件反射路径。
...
首面反射响应已经包含 `F`，折射响应使用互补权重 `1-F`；被选事件的条件 PDF 再乘 `0.5`，
使两帧/相邻像素的期望仍为两路贡献之和。后续再次命中透明材质时不会扩张路径树，而是从
RoboCute 完整透射闭包随机选择一个反射或折射事件；只有实际折射事件更新介质栈。

首面折射 proposal 为纯 delta。后续零粗糙度透明面随机选择一个 delta 反射或折射事件，不执行
NEE，也不增加 Russian roulette 深度；质量模式的粗糙透明面恢复完整 sample/eval/PDF、太阳
与面光源 NEE/MIS，并按普通非 delta 顶点参与 Russian roulette。性能模式仍只主动采样太阳。

### 材质语义

- 玻璃把 Minecraft 颜色按原版 encoded-sRGB alpha 规则与纯白混合一次，再把线性结果解释
  为一米透射率，通过 `extinction = -log(T)` 写入实体介质栈；颜色只由实际介质内路径长度
  产生 Beer-Lambert 吸收，不再在每个界面乘表面滤色。玻璃板暂用同一映射，较短的实际
  厚度或薄壁闭包会使它比一米厚玻璃更透明。
...
NEE 阴影 payload 同时携带 RGB 光学厚度和真实遮挡距离：

- 起点在介质中时，以当前栈顶消光乘 `tMax` 初始化光学厚度；
- 每个实体玻璃或水边界按自己的 RGB 消光累计带符号的剩余距离：进入边界加
  `extinction × (tMax-t)`，退出边界减同一项，最后计算 `exp(-opticalDepth)`；这里的
  `tMax` 是 payload 显式保存的原始光源距离，不能使用 any-hit 中等于当前候选距离的
  `gl_RayTmaxEXT`；
- 玻璃阴影以图元 UV 范围中心的 mip-0 texel 作为位置不变的一米介质参考，不让命中处的
  贴图边框与装饰 alpha 改变介质参数；Vanilla 彩色玻璃因此取主体 alpha `102/255`，普通
  玻璃取透明主体 alpha `0`，同一材质的浅深变化只来自直线穿透长度；
...
两种实时模式的 resolve 因此把辐射采样与重建观测分开：透明像素重新确认首接口后，独立
...
质量与性能模式复用同一组独立探针。性能模式的透射探针链受用户设置的最大表面顶点数约束，
并保持 80 字节单槽路径记录；质量模式的探针上限与完整路径预算一致，并保留两套 96 字节
重建槽，但每帧也只有棋盘格选中的一槽进入队列。反射探针额外追踪一次平面镜像目标。NRD
因而在两种棋盘格单元上都获得稳定的透射/反射表面身份，DLSS RR 同时获得上一帧镜像目标
或天空方向来生成 reflection motion。
...
| 层级 | 主要检查 |
| --- | --- |
| `PrimeBsdfGpuTest` | 首面 `(1-F)/η²` 折射保持确定性、后续完整闭包反射/折射 sample/eval/PDF、TIR、实体玻璃/水介质栈 |
| `PrimeProductionMathGpuTest` | 玻璃一米透射率与消光、RGB 直线光学厚度、PSR 数学、NRD pack/demod/ReJitter、FSR depth/motion/mask、自动曝光分桶/目标/适应/反照率修正 |
...
每条有效 NRD 分支只在当前像素执行一次 SG resolve。除此之外，两种实时模式的透明像素在
...
因此普通不透明像素 resolve 一次，透明像素最多 resolve 主分支与反射分支各一次；质量模式
