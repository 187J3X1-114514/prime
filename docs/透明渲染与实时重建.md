...
透射探针的上限与完整路径预算一致，并保留两套 96 字节路径/重建槽；两个首面分支都进入
...
每像素一个 80 字节记录扩为两个连续 SoA 槽。两套 96 字节路径槽和双路重建图像原本已经
存在；新增 wavefront backing 为 `104 B × 内部分辨率像素数`。精确 3840×2160 下 backing
由约 2468 MiB 增至约 3291 MiB。该布局以显存换取不增加串行 RT dispatch 和 CPU/GPU 调度。
