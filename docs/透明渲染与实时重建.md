...
| 管线与发行契约 | 两个 any-hit stage、六个 hit group、发行清单声明的全部生产 SPIR-V（当前 70 个）及逐模块验证 |
...
物理上每像素只保留 9 个队列索引。命中 surface scratch 为两个连续的 96 字节 SoA 槽。
两套 112 字节路径槽以 f32 保存两个介质的 RGB 消光与 IOR；
相对旧的 96 字节槽增加 `32 B × 内部分辨率像素数`，精确 3840×2160 为 253.125 MiB。
当前完整 wavefront backing 为 `468 B × 内部分辨率像素数` 加 96 字节命令区，4K 为
3,881,779,296 bytes，即约 3701.95 MiB。RG32F reflection motion 另比 RG16F 增加
