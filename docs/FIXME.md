...
诊断中曾观察到约一百万个已分配的灯光树 `Bounds`、数万节点和约 4.6 万个单次 TLAS Section 实例。该统计包含尚未回收的不可达对象，说明的是分配风暴，不应直接解读为等量的永久泄漏。当前性能分支已经复用 SAH bin 工作区、直接打包灯光树、流式写入 TLAS 实例并去掉 mesh 合并的中间大数组；这些改动降低了常数分配量，但全量重建的渐近成本仍然存在。
