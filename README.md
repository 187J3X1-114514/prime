...
- 每像素每帧追踪一个样本。主表面的漫反射与镜面光传输分别去调制后交给随包提供的 NRD 4.17.4 `REBLUR_DIFFUSE_SPECULAR`。首个可见透明界面只有在线性粗糙度严格为零时才确定性分叉反射与透射，并分别使用独立的 `REBLUR_DIFFUSE` 虚拟表面、运动向量和历史；非零粗糙度材质只采样一条完整 BSDF 路径，不分叉。透明结果完成后再与可见表面和稳定的天空/太阳 miss 在线性 HDR 中合成。
