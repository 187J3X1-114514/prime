...
vec3 primeGlassSurfaceTransmittance(vec3 baseColor, float opacity) {
    ...
    // Vanilla blends encoded sRGB. Gamma 2 is a cheap approximation to that operation in Prime's
    // linear working space and is intentionally stronger than a linear-light mix.
    vec3 encodedBlend = mix(vec3(1.0), sqrt(linearColor), coverage);
    return encodedBlend * encodedBlend;
}
