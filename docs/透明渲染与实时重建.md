...
2. **随机选择反射或折射会严重干扰降噪。** 两种事件指向不同半球、替代表面、深度、运动
   和有效方向。若逐样本按 Fresnel 随机切换，同一像素的 NRD/RR 输入会在两种不相容的
   几何历史间跳变。首接口固定同时生成两路并分别保存历史，后续接口固定折射，使两类信号
   的身份和 guide 随时间保持稳定。
...
因此最终语义选择“首透明接口固定求值折射与反射、后续透明接口固定折射、NEE 阴影不
折射只滤光”。它牺牲物理完备性，换取可解释的能量规则、稳定的重建信号和有界的实时成本。
...
分支后续再次命中透明材质时固定选择折射，不递归扩张为路径树。首面反射响应已经包含
`F`，折射响应使用互补权重 `1-F`；两支都执行，所以不再乘随机分支选择概率。后续界面
不再计算 Fresnel 分支；只有不存在折射解的全反射条件会回退到唯一有效的镜面方向。

折射支和所有后续透明顶点都是纯 delta 事件，不执行 NEE，也不增加 Russian roulette
...
- 玻璃每个表面命中乘
  ...
  原版 sRGB alpha 混色的大致强度；玻璃不进入介质栈，厚玻璃通常在进入、退出各滤色一次。
- 水面因子恒为白色，折射穿面时在进入/退出更新介质栈；已有实测水吸收继续按实际
  水中路径长度应用。
- 玻璃和水在首面之后不再乘 Fresnel。IOR 参与每个非薄壁透明面的 Snell 方向，并参与
  相机首透明面的反射能量；透射 throughput 不恢复物理闭包的 `/η²` 响应。
- 树叶薄壁透射保留原闭包，不按玻璃处理。

这是可解释但非物理的路径历史相关材质模型。原始积分器仍可对这个明确定义的有限反弹
模型给出无额外统计偏差的估计；该声明不等于真实物理正确，也不涵盖被确定性调度排除的
完整介质模型，更不适用于实时重建结果。
...
| 层级 | 主要检查 |
| --- | --- |
| `PrimeBsdfGpuTest` | 首面 `1-F` 折射、Snell/eta、粗糙反射 sample/eval/PDF、后续确定性折射、TIR、玻璃滤色和水栈 |
...
- 透明/染色厚玻璃是否逐面滤色，相机位于玻璃内、叠层玻璃及进入/退出方向是否符合
  当前确定性折射语义；
...
每条有效 NRD 分支只在当前像素执行一次 SG resolve。透明组合相对不透明路径的主要额外
成本来自条件反射的第二条 REBLUR 分支，而不是 camera-jitter 空间重采样。
