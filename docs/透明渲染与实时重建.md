...
只要实际离散反射/透射事件与规范 guide 一致，就直接复用该分支的精确 f32 continuation，首次
事件分叉、运动或非法状态才在 resolve 回放对应的独立探针。NEE 阴影不折射，只累计直线吸收。
...
flowchart TD
    ...
    R --> LF
    LF --> M["离散事件一致时复用；首次分叉才回放对应 guide"]
    M --> S{"实时重建后端"}
...
开始 Russian roulette。透射事件把 `relativeEta²` 累积到 `etaScale`，实时与离线存活率统一为
...
实时路径首先让辐射分支与规范重建身份共享离散链。判定只比较离散反射/透射事件和介质栈的
push/pop 层数，不比较方向、条件 `response / PDF` 或介质系数的浮点位；完整闭包与规范 guide
通过不同求值图得到同一个理想 delta continuation，位比较会错误拒绝这些可复用路径。共享期间
guide 采用 transport 实际提交的完整 f32 射线，不做 epsilon 或量化匹配，并在首个稳定的
solid-angle guide 前暂停 RR。首次事件分叉、运动或非法状态会标记对应身份槽，resolve 才从
首接口回放原有有界 GuideWalker；此后 transport 恢复普通 RR，独立探针拥有稳定重建身份。

回放探针不修改主路径的 throughput、radiance、介质栈或随机状态，但其自身会从相机到首接口
开始按每段介质长度累计 Beer-Lambert 透射率，使目标反照率与已吸收的辐射保持同一重建语义。
未分叉的静止路径直接使用分支捕获，透射和反射槽分别保存自己的 guide。
...
| --- | --- | --- |
| NRD-FSR | 启用 | 启用 |
| DLSS RR | 启用 | 分支捕获或按需回放的反射 walker 只提供 reflection MV，不提交第二主表面 |
| 禁用后处理 | 禁用 | 禁用 |

两个回放 probe 的上限都与完整路径预算一致，并复用已有透明 resolve、两套 112 字节路径/重建槽
和现有图像，不增加 dispatch 或持久显存。静止且离散事件一致的槽不发射 probe；最坏分叉情况
仍会增加最多七次反射 probe traversal。NRD 因而获得稳定的两套表面身份，DLSS RR 同时获得
上一帧反射 virtual target 或天空方向来生成 reflection motion。
...
每条有效 NRD 分支只在当前像素执行一次 SG resolve。实时路径仅为发生离散事件分叉、运动或
非法状态的身份槽在 resolve 中重追首接口并运行有界 guide 链；未分叉静止槽直接复用 wavefront
分支。探针只换取重建身份和 motion 稳定性，不增加辐射样本；成本应按透明像素覆盖率与实际
回退率单独测量。
