...
- **PBR 材质支持**：支持部分 LabPBR 1.3 数据，包括粗糙度、金属、介电反射和发光信息；
- **动态场景**：路径追踪场景会接收地形、实体、方块实体、移动方块和文字等动态内容；
- **实时降噪与重建**：首要支持 NVIDIA DLSS Ray Reconstruction，并为其他兼容显卡提供
  NRD + FSR；
- **SDR 与 HDR 输出**：使用统一的 AgX-HSV 显示变换；HDR 会查询当前屏幕的亮度能力，
  以线性 scRGB 延伸高光，并支持自动或手动标定参考白；
- **反弹光照缓存**：可选的 SHARC 缓存会记住附近已经算过的反弹光照，让实时路径追踪通常
  更快、噪点更少；它会额外占用约 256 MiB 显存，不支持的显卡会自动关闭；
- **离线渲染模式**：冻结当前场景并在原生分辨率累积未经降噪的原始样本。它能保留更多
  细节并避免实时重建伪影，适合追求质量更高的最终结果；但通常比实时画面噪点更多，需要
  更长时间才能收敛；
- **纹理几何位移**：实验性地将材质高度信息转换为真实表面起伏，会增加场景构建和显存开销；
...
- **SHARC 辐射缓存**：复用附近已经算过的反弹光照，通常建议保持开启；显存紧张时可以关闭；
...
- 透明介质最多可靠嵌套两个非空气区域。更深嵌套、非封闭模型和无法判断内外的边界不保证
  正确吸收或折射；
- 实时透明照明在首接口采用固定双槽和直线阴影滤光；完整模式保留条件双分支并在后续采用
  有界单分支，轻量模式保留相同的首面条件透射/反射，但后续透明界面固定折射（TIR 除外），
  并取消独立透明 guide 探针；轻量分支会在捕获首个稳定 anchor 前暂停 RR，均不完整求解任意
  多层折射光路；
