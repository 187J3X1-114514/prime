...
| MIG-002 | BSDF 适配层诊断 | 无显式状态的空诊断钩子无法观察 adapter 内产生后又被拒绝或净化的异常值。 | 新增逐 invocation `PrimeNumericalObservation`，在样本拒绝和 denoise albedo 钳制之前分类原始值，再由 realtime/offline integrator 提交到当前 path 诊断；禁用 observer 不引入隐藏状态。`PrimeBsdfDiagnosticsGpuTest` 覆盖 NaN、Inf、负值、非法方向和禁用路径，完整 BSDF 数值/分布/能量套件通过。 | 已解决 |
