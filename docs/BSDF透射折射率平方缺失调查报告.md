...
实时质量与性能模式在首接口以 50/50 棋盘格选择一个条件反射或确定性折射事件；后续透明
顶点与离线 renderer 都使用完整 RoboCute 透射闭包的随机单分支，因此 `/η²`、事件 PDF、
`relativeEta` 和介质栈契约进入实时生产路径。首接口的条件折射适配也显式应用与事件
`relativeEta` 一致的 `/η²`；它仍不直接等同于完整 rough-transmission closure。
