...
- 只表达一层薄壁空气接口的反射切割，不表达可解析厚度、平行面间多次反射、薄膜干涉、
  色散或腔内吸收；
- 粗糙度只来自现有材质/薄壁闭包能力；新增能力必须作为精确 compact OpenPBR topology 实现；
...
因此最终语义选择“首透明接口固定求值透射与反射、后续透明接口按完整闭包随机单分支、NEE
阴影不折射只累计直线吸收、两路重建使用独立探针”。它保住首可见面的稳定重建身份，并消除
首面事件轮换在细线、遮挡边缘和低能反射上留下的时空采样缺口。

```mermaid
flowchart TD
    A["相机射线首命中透明接口"] --> B["固定材质分叉"]
    B --> T["条件折射 wavefront：1-F"]
...
选择概率。后续再次命中透明材质时不会扩张路径树，而是从 compact
OpenPBR 完整透射闭包随机选择一个反射或折射事件；只有实际折射事件更新介质栈。

零粗糙度透明面的条件 proposal 为 discrete-only。后续零粗糙度透明面随机选择一个 discrete 反射或
...
`min(sqrt(max(throughput) × etaScale), 1)` 的平方根度量，与离线传输一致；不再为透明链维护独立 RR 深度。
...
- 阴影起点只应用一次自相交偏移，不再叠加正 `tMin`。离线延迟 Area 阴影沿光线方向偏移时
  同量缩短 `tMax`，使遍历终点仍停在采样光源而不会误收光源背后的遮挡；
...
实时 resolve 因此把辐射采样与重建观测分开：透明像素重新确认首接口后，用同一个有界
...
遍历从法线偏移后的 origin 发射，但 PSR 累计 `surface.t`，不把仅用于防自相交的 offset
算入光学路径。静止的多层镜面链因此仍严格落回当前相机射线，而不需要单跳平面镜像特判。

动态命中在原有 emitter/LOD 两个空闲槽中保存全局 triangle id 和 UNORM16 重心坐标，再从已有
上一帧 f32 顶点缓冲重建 previous position 与 geometric normal。DLSS RR 会消费由它们构造的
...
主 diffuse albedo 置零；全部辐射与 virtual motion 只进入反射槽。它不会继续携带一份虚假的
透射材质，也不会把 TIR 方向交给折射重投影。
...
TIR 是例外于“回退到可见接口但仍保留透射材质”的情况：主 noisy-diffuse、透射 guide 和
diffuse albedo 都为空，反射信号继续使用可见接口的反射材质与独立 reflection motion。
...
透射和反射已经在路径积分器内以固定条件双分支积分，没有一张语义真实的
...
当前完整 wavefront backing 为 `448 B × 内部分辨率像素数` 加 96 字节命令区，4K 为
3,715,891,296 bytes，即约 3543.75 MiB。RG32F reflection motion 另比 RG16F 增加
`4 B × 内部分辨率像素数`，4K 为 31.64 MiB。两项都不增加串行 RT/compute dispatch 或
