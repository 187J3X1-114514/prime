...
本文记录 Prime 当前透明主路径从首命中分叉，到 Noisy、NRD-FSR 或 DLSS Ray
...
2. **在首可见面随机选择反射或折射会严重干扰降噪。** 两种事件指向不同半球、替代表面、
   深度、运动和有效方向。若首面逐样本按 Fresnel 随机切换，同一像素的 NRD/RR 输入会在
   两种不相容的几何历史间跳变。首接口因此固定同时生成两路并分别保存历史；每路后续命中
   透明面时只随机延续一条路径，不再改变首可见面的 guide 身份。
...
因此最终语义选择“首透明接口固定求值折射与反射、后续透明接口按完整闭包随机选择单分支、
NEE 阴影不折射只滤光”。它保住首可见面的稳定重建身份，并在不扩张路径树的前提下恢复
后续界面的 Fresnel 反射。

```mermaid
flowchart TD
    A["相机射线首命中透明接口"] --> B["固定材质分叉"]
    B --> T["折射 wavefront：1-F"]
    B --> R["条件反射 wavefront"]
    T --> C["后续透明面：完整闭包随机单分支"]
    R --> C
    C --> M["合并原始结果；resolve 独立探测透明 guide"]
    M --> S{"实时重建后端"}
...
- 槽 0：按几何法线与界面 IOR 确定方向、响应为 `(1-F) × 表面透射因子` 的折射路径；
- 槽 1：条件反射路径。
...
首面反射响应已经包含 `F`，折射响应使用互补权重 `1-F`；两支都执行，所以不乘随机分支
选择概率。任一分支后续再次命中透明材质时不会递归扩张路径树，而是从 RoboCute 完整透射
闭包随机选择一个反射或折射事件；分支选择概率保留在 PDF 中，只有实际折射事件更新介质栈。

首面折射支为纯 delta。后续零粗糙度透明面随机选择一个 delta 反射或折射事件，不执行 NEE，
也不增加 Russian roulette 深度；粗糙透明面恢复完整 sample/eval/PDF、太阳与面光源 NEE/MIS，
并按普通非 delta 顶点参与 Russian roulette。首面粗糙反射仍使用现有条件 GGX 分支。

### 材质语义

- 玻璃每个实际折射事件乘
  `square(mix(白, sqrt(linear Rec.2020 基础色), alpha))`。该 gamma 2 近似用于恢复
  原版 sRGB alpha 混色的大致强度；连续穿过厚玻璃时通常在进入、退出各滤色一次，反射事件
  不滤色。
- 水面因子恒为白色，只有采到折射时才在进入/退出更新介质栈；已有实测水吸收继续按实际
  水中路径长度应用。后续完整闭包也维护用于 IOR 的零吸收玻璃栈项。
- 首面固定折射使用适配层的几何 Snell 方向与互补 Fresnel 权重；后续随机单分支恢复完整
  闭包的 Fresnel、PDF、`relativeEta` 与透射 `/η²` 响应。
- 树叶薄壁透射保留原闭包，不按玻璃处理。

这是面向重建的混合估计器：首可见面使用确定性双分支，后续顶点使用完整闭包单分支。
它不等于完整物理介质传输，尤其没有改变直线透明阴影近似，也不适用于评价重建后的统计偏差。
...
阴影路径不计算透明面的折射、后续 Fresnel 损失或反射分支，始终沿原 NEE 线段滤光。
...
完整 Wavefront 的 resolve 因此把辐射采样与重建观测分开：透明像素重新确认首接口后，独立
追踪一条确定性平滑透射 guide 和一条首接口平面反射 guide。探针不修改 throughput、radiance、
介质栈或随机路径状态；已有分支捕获只作为探针无效时的回退。
...
轻量 Wavefront 的辐射传输仍以 50/50 时空棋盘格只选择反射或折射之一，但 resolve 与完整
模式复用同一组独立探针。透射探针链受轻量最大表面顶点数约束，反射探针额外追踪一次平面
镜像目标；两者不进入 80 字节路径记录，也不改变当前帧选择的辐射分支。NRD 因而在两种
棋盘格单元上都获得稳定的透射/反射表面身份，DLSS RR 同时获得上一帧镜像目标或天空方向
来生成 reflection motion。
...
| 层级 | 主要检查 |
| --- | --- |
| `PrimeBsdfGpuTest` | 首面 `1-F` 折射保持确定性、后续完整闭包反射/折射 sample/eval/PDF、TIR、玻璃滤色和介质栈 |
| `PrimeProductionMathGpuTest` | gamma 2 玻璃滤色端点、相对线性混色强度、滤色乘积、乱序水段、PSR 数学、NRD pack/demod/ReJitter、FSR depth/motion/mask、自动曝光分桶/目标/适应/反照率修正 |
...
- 透明/染色厚玻璃是否只在折射时逐面滤色，相机位于玻璃内、叠层玻璃及进入/退出方向是否
  符合首面确定性分支与后续随机完整闭包语义；
...
每条有效 NRD 分支只在当前像素执行一次 SG resolve。除此之外，完整 Wavefront 的透明像素
在 resolve 中增加一次首接口重追踪、一次平面反射追踪和最多八层的有界透射 guide 链。
...
因此普通不透明像素 resolve 一次，透明像素最多 resolve 主分支与反射分支各一次；非透射
...
| wavefront 分叉、推进与合并 | `shaders/realtime_wavefront.rgen`, `shaders/wavefront_state.glsl` |
