...
闭包。对于相接、非水、光滑、实体透明材质且两侧有效 IOR 相等的边界，关系解析可将它标为
`THIN_AIR_FILM`：在同一几何事件上用现有薄壁 Fresnel/粗糙度能力表达一层分辨率以下的空气
裂隙，然后继续原介质传输。关闭该选项时，相同端点退化为无可见内部边界。

这是有意保守的最小规格：

- 只表达一层薄壁空气接口的反射切割，不表达可解析厚度、平行面间多次反射、薄膜干涉、
  色散或腔内吸收；
- 粗糙度只来自现有材质/薄壁闭包能力；完整实时和离线模式的新增能力必须作为精确 compact
  OpenPBR topology 实现，轻量模式只允许映射到已声明的 [LitePBR](LitePBR.md) 低阶拓扑；
...
| 反射替代表面 | 沿条件反射路径遇到的第一个可用 solid-angle 表面或方向 | 反射 NRD 历史；RR 使用独立平面探针运动 |
...
视距场景预算难以同时承担这些成本。完整实时 specialization 保留现有灯光树、长尾队列和
完整 NEE。轻量 specialization 在 solid-angle anchor 使用单候选功率 alias 方块光 NEE，但不
遍历灯光树或建立 RIS 候选；透明 discrete 链仍跳过 NEE。它保留首界面的原条件透射/反射双分支，
随后遇到的透明界面固定为确定性折射续接（TIR 除外），只保留首界面的反射效果，并直接复用
辐射分支内捕获的 guide，不执行独立透明探针。

两种模式都保留首透明接口的条件透射与反射。完整模式在后续透明接口按完整闭包随机单分支，
并以两条独立探针重建身份；轻量模式让两个首面分支在后续透明接口都沿折射链 probe 的确定性
路径续接，不再支付 probe traversal，也不再产生后续透明反射。两者的 NEE 阴影都不折射，只
累计直线吸收。
...
flowchart TD
    ...
    T --> L{"实时模式"}
    R --> L
    L -->|完整| LF["后续透明面：完整闭包随机单分支"]
    LF --> M["合并原始结果；resolve 独立探测透明 guide"]
    L -->|轻量| LL["后续透明面：固定折射；仅 TIR 反射"]
    LL --> ML["稳定 anchor 前暂停 RR；直接使用分支内 guide"]
    ML --> S
    M --> S{"实时重建后端"}
...
选择概率。后续再次命中透明材质时不会扩张路径树：完整模式从 compact OpenPBR 闭包随机
选择一个反射或折射事件；轻量模式使用 LitePBR 接口并固定选择折射，只有 TIR 改为反射。
只有实际折射事件更新介质栈。

零粗糙度透明面的条件 proposal 为 discrete-only。完整模式的后续零粗糙度透明面随机选择一个
discrete 反射或折射事件且不执行 NEE；粗糙透明面的首面条件分支和后续完整闭包都提供一致的
sample/eval/PDF。所有实际散射（包括 discrete-only 透明界面）统一计入 bounce，并从第二次散射
...
完整模式的实时 resolve 因此把辐射采样与重建观测分开：透明像素重新确认首接口后，用同一个有界
GuideWalker 分别追踪固定的首接口透射、反射两个身份槽。探针不修改主路径的
throughput、radiance、介质栈或随机状态，但其自身会从相机到首接口开始按每段介质长度累计 Beer-Lambert
透射率，使目标反照率与已吸收的辐射保持同一重建语义；已有分支捕获只作为探针无效时的回退。

轻量模式不运行 GuideWalker。首面的透射与反射使用 LitePBR 条件闭包；之后辐射路径遇到的透明
界面使用 GuideWalker 相同的确定性折射续接。分支在找到首个稳定的 solid-angle guide 和
transmission anchor 前禁止 RR，捕获后立即恢复普通 RR；resolve 只读取这两条分支的持久结果。
这样不增加独立 probe ray，但透明反射只保留首可见接口，粗糙后续透明反射、后续 Fresnel 损失
和探针重建的动态反射 virtual motion 都是明确舍弃的近似。分支捕获的反射 virtual target 仍
直接提供 RR specular motion；首接口 TIR 的有限目标复用不存在的透射主 guide 槽，因此不会
回退到可见玻璃接口。当前分支 PSR 不保存上一帧 delta 链，移动透明接口或移动反射目标仍回退
到当前 virtual target；静止目标不需要独立探针。

GuideWalker 只折叠 delta 事件：光滑不透明导体继续反射；光滑透明边界采用固定的透射
continuation，使入射/出射成对边界后的场景仍是 guide 目标，TIR 因不存在折射方向而继续反射。
这个选择只决定重建身份，不修改随机辐射路径，也不根据能量或 RNG 交换两个首接口身份槽。
粗糙界面和其他 solid-angle 闭包作为终止表面。
...
可见接口。探针与回退表面都通过同一 `primeNrdGuideControl` 编码重建材质类别，导体 Fresnel
...
| --- | --- | --- |
| NRD-FSR | 启用 | 启用 |
| DLSS RR | 启用 | 独立反射 walker 只提供 reflection MV，不提交第二主表面 |
| 禁用后处理 | 禁用 | 禁用 |

两个 probe 的上限都与完整路径预算一致，并复用已有透明 resolve、两套 112 字节路径/重建槽
和现有图像，不增加 dispatch 或持久显存。相比单跳反射探针，最坏情况会增加最多七次反射
probe traversal；常见链在首个 solid-angle 表面终止。NRD 因而获得稳定的两套
表面身份，DLSS RR 同时获得上一帧反射 virtual target 或天空方向来生成 reflection motion。

## 3. 公共输出边界

`shaders/realtime/wavefront_output.slang` 是积分器与实时重建之间的适配层。积分器只返回一个原始模型样本；
...
反射 guide 无效时只把 `reflectionMaterial.a` 写为负值。`shaders/post/nrd_motion.compute.slang` 在读取其他
...
`shaders/post/nrd_composite.compute.slang` 负责三件事：
...
Rec.2020 到显示 sRGB 的 AgX HSV 变换。
...
首接口 TIR 且反射 walker 命中有限 solid-angle 表面时没有并行透射身份。RR 将整条有界
...
| 管线与发行契约 | 两个 any-hit stage、六个 hit group、发行清单声明的全部生产 SPIR-V（当前 60 个）及逐模块验证 |
...
首面固定双分支保持现有 pass 数，让两条 continuation 在同一批 wavefront 阶段中并行推进。
为容纳最坏情况下的同时活动路径，六条队列都从 `N` 扩为 `2N`，命中 surface scratch 从
每像素一个 80 字节记录扩为两个连续 SoA 槽。两套 112 字节路径槽以 f32 保存两个介质的 RGB 消光与 IOR；
相对旧的 96 字节槽增加 `32 B × 内部分辨率像素数`，精确 3840×2160 为 253.125 MiB。
当前完整 wavefront backing 为 `480 B × 内部分辨率像素数` 加 96 字节命令区，4K 为
3,981,312,096 bytes，即约 3796.88 MiB。RG32F reflection motion 另比 RG16F 增加
...
| 阶段 | 主要文件 |
| --- | --- |
| 透明 BSDF 条件 proposal | `shaders/bsdf/adapter/bsdf.slang`, `shaders/integrator/integrator.slang` |
| NEE 玻璃/水滤光 | `shaders/rt/shadow.anyhit.slang`, `shaders/rt/shadow_payload.slang`, `shaders/material/transparent_material.slang` |
| wavefront 分叉、推进与合并 | `shaders/realtime/realtime_wavefront*.raygeneration.slang`, `shaders/realtime/wavefront_state.slang` |
| 独立透明探针与 PSR 数学 | `shaders/realtime/transparent_guide_probe.slang`, `shaders/core/queued_psr_math.slang` |
| 后端输出适配 | `shaders/realtime/wavefront_output.slang` |
| NRD/FSR 输入准备 | `shaders/post/nrd_motion.compute.slang`, `shaders/post/fsr_input.slang` |
| NRD SH 官方应用侧解析 | `shaders/core/nrd_sh.slang` |
| 透明 phase restore 与组合 | `shaders/post/nrd_composite.compute.slang` |
| NRD native 配置 | `native/nrd/prime_nrd_bridge.cpp` |
| FSR 调度 | `NrdFsrPostProcessor`, `Fsr3Upscaler`, `FsrDispatchPlan` |
| RR 输入准备 | `shaders/post/rr_prepare.compute.slang`, `DlssRrPreparePass` |
