...
折射事件，不执行 NEE，也不增加 Russian roulette 深度；粗糙透明面的首面条件分支和后续完整
闭包都提供一致的 sample/eval/PDF。质量模式对它执行太阳与面光源 NEE/MIS，并按普通非 delta
顶点参与 Russian roulette；性能模式仍只主动采样太阳。
...
NEE 阴影 payload 同时携带 RGB 光学厚度矩、非水终点介质消光、水边界绕数和真实遮挡距离：

- 起点在介质中时，只以当前栈顶消光初始化终点介质状态，不预乘 `tMax`；
- 每个实体玻璃或水边界按自己的 RGB 消光累计带符号的一阶矩：进入边界加
  `-extinction × t`，退出边界加 `extinction × t`。玻璃相应增减终点消光；水则增减整数
  绕数，最终光学厚度为
  `moment + (terminalExtinction + waterExtinction × waterWinding) × tMax`。这与按区间长度积分等价且不依赖
  any-hit 调用顺序，同时避免水下太阳射线用两个 `extinction × 1000000` 大数相减恢复
  几格水程所产生的 RGB 浮点消去；
- 非水起点介质与边界消光都先规范化为 Wavefront 记录使用的 fp16 表示。水的起点状态与
  水面边界不再做 RGB 浮点相减，只通过整数绕数抵消；因此队列解包值与边界常量之间的逐
  通道微小差异不会再被太阳的百万单位 `tMax` 放大并把青色滤光翻成黄色；
- 玻璃阴影以图元 UV 范围中心的 mip-0 texel 作为位置不变的材质分类和颜色参考；染色玻璃
  始终使用 alpha `0.4` 的消光，不让粗糙边框的 alpha 改变介质参数；普通玻璃主体消光为零，
  但其 alpha `> 0.5` 的漫反射边框作为真实不透明遮挡；
