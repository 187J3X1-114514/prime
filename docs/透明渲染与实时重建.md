...
不再回落到 FP16 motion scratch。缺少有效上一帧位置时仍安全回退当前位置。NRD 仍使用自己的
两分支 raw-position 契约。

平面反射探针沿首接口的确定性 discrete 反射方向追踪一次。有限目标命中时，将当前与上一帧
目标点分别关于当前/上一帧接口位置和法线镜像，得到可投影的 virtual position；遍历所需的
法线 origin offset 在镜像后显式消除，因此静止斜射平面仍严格落回当前相机射线。命中天空时
保存平移无关的 virtual direction，并在动态接口上使用上一帧平面法线。粗糙反射不采用单一
平面虚像。
...
透射探针的上限与完整路径预算一致，并保留两套 112 字节路径/重建槽；两个首面分支都进入
队列。反射探针额外追踪一次平面镜像目标。NRD 因而每帧都获得稳定的透射/反射表面身份，
DLSS RR 同时获得上一帧镜像目标或天空方向来生成 reflection motion。
...
RR scratch 用 R32F 主距离的符号编码“上一帧位置有效”，反射粗糙度保持原始非负值；这避免
