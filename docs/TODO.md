...
- 将主可见性分类从两套完整路径积分中拆出，并压紧透明像素后再调度反射/透射路径。当前透明覆盖的
  像素会先计算一条忽略玻璃的完整不透明路径，随后其结果又被透明合成覆盖；透明 raygen 也会为
  全屏再次查询首界面。共享首命中与保持原样本编号可以让估计器数学不变，但必须同时保留“最近
  真实透明界面”和“忽略透明后的不透明 NRD 引导”，还要处理压紧队列、间接 dispatch、历史清理
  与边界滤波，因此本质上是一次小型 wavefront/可见性架构重写，不适合作为当前源码化简。
