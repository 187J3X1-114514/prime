...
- 每像素每帧只追踪一条完整 BSDF 路径。NRD 4.17.4 的 `REBLUR_DIFFUSE_SPECULAR` 以 63 帧主历史、10 帧快速历史和 4 帧 history-fix 处理面积光与间接光，太阳直射则拆成未遮挡光照与首遮挡距离，由同一 NRD instance 中的 `SIGMA_SHADOW` 专门重建阴影。PT 反弹循环以 `hitted_non_delta=false` 开始，沿透明 delta 链继续推进；每个 delta 事件只把当前表面的镜面 albedo 乘入独立 guide 累积量，首次采到 non-delta 事件时再乘入该表面的漫反射与镜面 albedo 之和，并锁定降噪法线与线性粗糙度。REBLUR 命中距离重建使用 5×5 区域。透明材质不再确定性拆成反射、透射两条路径，也不再拥有额外的反射/透射降噪实例、虚拟表面历史或透明合成 pass。
