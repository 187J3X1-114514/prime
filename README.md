...
- 实时 1 spp 路径追踪，包括太阳、天空、全天星空和发光方块照明；
...
Prime 会读取切线空间法线，但当前仍使用几何法线；法线纹理 alpha 中的高度仅用于可选的
纹理体素表面，AO 和孔隙率尚未参与着色。
...
- 相机首透明面固定混合确定性折射与 Fresnel 反射，完整 Wavefront 的后续透明面从完整
  闭包随机选择一条反射或折射路径；玻璃只在实际穿过表面时使用 gamma 2 近似原版 sRGB
  alpha 混色强度；实时重建 guide 由独立确定性透射/平面反射探针提供，不改变随机辐射；
  NEE 阴影不折射，只累计同一透明滤光；
- 只有水产生体积吸收；后续完整闭包可为非薄壁玻璃保存零吸收 IOR 栈项。透明边界需有
  正确法线和可解释的进出拓扑，最多可靠嵌套两个非空气透明区域；
- 部分资源包的红石火把头部可能被错误识别为不透明几何；
- 高视距持续加载复杂地形时仍可能出现较高 Java 堆分配和 GC 停顿；
- 部分实体可能闪烁或留下历史拖影，物品展示框内容尚未进入光追场景；
- 水下阴影、信标光柱遮光和两层以上水体嵌套仍有已知限制。
