...
Prime 当前的实体介质栈固定容纳两个非空气介质，并依赖表面法线正确、进入与退出严格遵循
LIFO 顺序。空气不占栈，因此空气→玻璃→水及其反向退出属于支持范围；空气→玻璃→水→冰
需要同时保存三个实体介质，超出支持范围。

第三层介质的当前界面仍可能使用进入前的正确栈顶计算相对 IOR，但该介质无法入栈。此后的
体积吸收、退出界面的外部 IOR、Radiance `/η²` 和 pop 顺序均不再保证正确。介质重叠、
非流形边界、遗漏界面或非 LIFO 退出同样不在支持契约内。相机初始介质只特殊识别水，不保证
从玻璃、冰或其他实体介质内部开始时正确。
...
玻璃、水等方块与其它方块相接的面，或者两个不同颜色玻璃的相接面经常渲染错误，考虑是否剔除掉这个面
