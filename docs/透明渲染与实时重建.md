...
本文只描述实时路径。参考累积截图会直接累积两条物理分支之和，不经过 NRD、FSR 或
DLSS RR。
...
| 透射替代表面 | 沿条件透射路径遇到的第一个可用非 delta 表面 | 主 NRD 历史或 RR 主表面 |
...
只有第一个可见透明接口分叉。分支后续再次命中透明材质时恢复普通单路径采样，不递归
扩张为路径树。因此额外工作有固定上界，同时首接口的 Fresnel 反射与透射都能在同一帧
得到真实样本。

零粗糙度透明接口是纯 delta 事件，不执行 NEE，也不增加 Russian roulette 深度。非零
粗糙度或混合透明按普通非 delta 顶点处理。
...
这个 anchor 让折射后的目标深度仍锚定在当前像素的可见主射线上。相机静止时，它投影回
当前带抖动样本位置，避免把折射横向偏移误报为物体运动。

如果没有遇到有效替代表面、delta 链溢出或平面交点退化，guide 回退到真实可见接口。
回退只影响重建 guide，不截断也不伪造物理传输。
...
`wavefront_output.glsl` 是积分器与实时重建之间的适配层。积分器只返回一个物理样本；
...
DLSS RR 是另一条完整后端，不先运行 NRD，也不再运行 FSR。它与 NRD-FSR 共享透明物理
分叉，但输出适配不同。
...
- `inputColor`：透射、反射、太阳和 stable radiance 的物理总和，再按可见接口应用大气；
...
| 层级 | 主要检查 |
| --- | --- |
| `PrimeProductionMathGpuTest` | PSR 数学、NRD pack/demod/ReJitter、FSR depth/motion/mask |
...
- ReJitter 的四邻域法线与 view-Z；
- 四个 coverage footprint 重叠的 3×3 compatibility grid。

因此普通不透明像素仍只 resolve 一次；额外四 tap 路径只在可见材质为 transmissive 时
启用。进一步优化必须证明不改变 per-tap resolve、中心 guide、不连续面过滤和权重归一化
的数学结果。实际收益应以 GPU benchmark 为准，不能只用源码 image-load 数量代替耗时。
