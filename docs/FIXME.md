...
玻璃板附近曾观察到边界突兀的阴影，但尚无稳定复现条件。需分别核对直线阴影滤光、几何边界
配对和体积栈，不能在证据不足时归因于其中任一项。

### 玻璃闪烁噪点

玻璃材质曾观察到随帧闪烁的离散噪点，但尚无稳定复现条件，当前也不能确认发生在传输采样、
确定性 guide、wavefront 状态、重建输入还是重建历史。后续复现时需同时记录材质与资源包、
重建后端、SHARC 状态、含噪输入和 guide 视图，并以固定相机与随机种子逐级定位；不能在证据
不足时归因于任一候选子系统。

### 斜水面发黑

斜水面曾出现极细密的发黑伪影，但尚无稳定复现条件。后续应记录资源包、太阳角度、相机距离、
重建后端和含噪输入，再区分几何、法线、透明采样与重建问题。
